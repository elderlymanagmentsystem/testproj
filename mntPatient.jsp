...
<%
	...
	ArrayList<BankBean> bankBeanList = patGrpBean.getBankBeanList();
...
<form name="mntPatient" action="mntPatient" method="post">
	...
	<div class="w3-panel w3-large">
		...
		<div id="enqScreen">
			...
			<form name="mntPatient" action="mntPatient" method="post"><s:token/>
				...
				<input type="hidden" name="funcId" value="<%=enqFuncId %>">
				<input type="hidden" name="patGrpBean.enqLivStatus" value="Y">
			</form>	
			</div>
		...
		<form name="mntPatient" action="mntPatient" method="post" enctype="multipart/form-data"><s:token/>
		<div style="display:none" id="addDetailScreen">
			...
			<div class="w3-bar">
				...
				<a class="w3-bar-item w3-button w3-grey mc-tabBtn" id="addTabBtn4" onclick="switchTabPage('add',5)"><i class="fas fa-file-invoice-dollar fa-fw"></i></a>
			...
			<div id="addTabPage1">
				...
				<tr>
			  	...
			  	<tr>
			  		<td><span class="w3-text-red mc-italic">中文姓名: <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[2][0]).getMsg() %></span></td>
			  		...
			  		<td><span class="w3-text-red mc-italic">英文姓名: <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[3][0]).getMsg() %></span></td>
			  	...
			  	<tr>
			  		<td><span class="w3-text-red mc-italic">身份證號碼:<%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[6][0]).getMsg() %></span></td>
			  		...
			  		<td><span class="w3-text-red mc-italic">性別: <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[7][0]).getMsg() %></span></td>
			  ...
			  <tr>
			  	<td><span class="w3-text-red mc-italic"><%= EmsDB.EM_PER_PERSONAL_PARTICULAR[8][1] %>: <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[8][0]).getMsg() %></span></td>
			  ...
			  <tr>
			  	...
			  	<td>
			  		...
			  		<img src="/EMS/<%= patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[12][0]).getFormValue() %>" class="mc-img">
			  		<% } %>
			...
			<div id="addTabPage2" style="display:none">
			  ...
			  <%	LivBean livBean = new LivBean(); 
			  	...
			  	<tr>
			  		<td><span class="w3-text-red mc-italic">床號: <%=livBean.getField(EmsDB.EM_LIV_RECORD[2][0]).getMsg() %></span></td>
			  		<td>
			  	...
			  	<tr>
			  		<td><span class="w3-text-red mc-italic">入住日期: <%=livBean.getField(EmsDB.EM_LIV_RECORD[5][0]).getMsg() %></span></td>
			  		<td>
			  			<div class="flatpickr date-container">
				  	...
				  	<td><span class="w3-text-red mc-italic"><%= EmsDB.EM_LIV_RECORD[7][1] %>: <%=livBean.getField(EmsDB.EM_LIV_RECORD[7][0]).getMsg() %></span></td>
			  	...
			  	<tr>
				  	<td><span class="w3-text-red mc-italic"><%= EmsDB.EM_LIV_RECORD[8][1] %>: <%=livBean.getField(EmsDB.EM_LIV_RECORD[8][0]).getMsg() %></span></td>
			  	...
			  	<tr>
				  	<td><%= EmsDB.EM_PER_PERSONAL_PARTICULAR[24][1] %>: <span class="w3-text-red mc-italic"><%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[24][0]).getMsg() %></span></td>
				  	<td colspan="3"><select name="<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[24][0] %>" style="width:150px;">
				  				<option value=""></option>
				  	<% 	for(int i=0;i<bankBeanList.size();i++){
				  			if(patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[24][0]).getFormValue().equals(bankBeanList.get(i).getBankId())){ %>
								<option value="<%=bankBeanList.get(i).getBankId() %>" selected><%=bankBeanList.get(i).getBankName() %></option>
							<% }else{ %>
								<option value="<%=bankBeanList.get(i).getBankId() %>"><%=bankBeanList.get(i).getBankName() %></option>
							<% }
				  		} %>
					</select>
				  	</td>
			  	...
			  	<tr>
				  	<td><%= EmsDB.EM_PER_PERSONAL_PARTICULAR[26][1] %>: <span class="w3-text-red mc-italic"><%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[26][0]).getMsg() %></span></td>
				  	<td><input type="text" name="<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[26][0] %>" value="<%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[26][0]).getFormValue() %>" maxlength="<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[26][3] %>" class="mc-fw" style="max-width:10em"></td>
				  	<td><%= EmsDB.EM_PER_PERSONAL_PARTICULAR[25][1] %>: <span class="w3-text-red mc-italic"><%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[25][0]).getMsg() %></span></td>
				  	<td><input type="text" name="<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[25][0] %>" value="<%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[25][0]).getFormValue() %>" maxlength="<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[25][3] %>" class="mc-fw" style="max-width:10em"></td>
			  	</tr>
			  	...
			  	<tr>
			  		...
			  		<td colspan="3"><div class="flatpickr date-container">
						...
						<tr>
							<td><span class="w3-text-red mc-italic">飲食種類  <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[17][0]).getMsg() %> <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[18][0]).getMsg() %></span></td>
						...
						<tr>
							<td><span class="w3-text-red mc-italic">護理服務 <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[19][0]).getMsg() %> <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[20][0]).getMsg() %></span></td>
						...
						<tr>
							<td><span class="w3-text-red mc-italic">護理程度 <%=patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[21][0]).getMsg() %></span></td>
			...
			<div id="addTabPage3" style="display:none">
				...
				<div style="overflow-x:auto;">
					<table id="depositeListTable">
						<tr>
					  		<td>通知順序</td>
					  		<td><span class="w3-text-red mc-italic">名稱</span></td> 
					  		<td>關係</td>
					  		<td><span class="w3-text-red mc-italic">電話</span></td>
					  		<td>電郵</td>
						</tr>
						<% 
			...
			<div id="addTabPage4" style="display:none">
				<table class="w3-table">
					<tr>
						<th class="color1 w3-xlarge">新入住 - 繳交按金<span class="w3-text-red mc-italic"><%=patGrpBean.getMsg() %></span></th>
					</tr>
				</table>
				<table class="w3-table-all" id="addDepositScreenTable">
				  	...
				  	<tr>
				  		<td><span class="w3-text-red mc-italic">按金金額: <%=transBean.getField(EmsDB.EM_TRA_TRANSACTIONS[9][0]).getMsg() %></span></td>
				  	...
				  	<tr>
						<td><span class="w3-text-red mc-italic">入賬日期: <%=transBean.getField(EmsDB.EM_TRA_TRANSACTIONS[8][0]).getMsg() %></span></td>
				  		<td>
				  			<div class="flatpickr date-container">
			...
			<div id="addTabPage5" style="display:none">
				<table class="w3-table">
					<tr>
						<th class="color1 w3-xlarge">新入住 - 上存檔案<span class="w3-text-red mc-italic"><%=patGrpBean.getMsg() %></span></th>
					</tr>
				</table>
				<table class="w3-table-all" id="addDepositScreenTable">
						<col width="10%">
						...
						<col width="10%">
						<col width="40%">
				  	<tr><td colspan="4" class="w3-xlarge mc-bold mc-underline">上存檔案</td></tr>
				  	<tr>
				  		<td>上存檔案</td>
				  		<td colspan="3"><input type="file" name="fileUpload2" style="max-width:400px"></td>
				  	</tr>			  	
				</table>
			</div>			
		...
		<input type="hidden" name="<%= EmsDB.EM_TRA_TRANSACTIONS[6][0] %>" value="按金">
	  	<input type="hidden" name="<%= EmsDB.EM_TRA_TRANSACTIONS[7][0] %>" value="D">
	  	<input type="hidden" name="<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[27][0] %>" value="Y">
		...
		<div style="display:none" id="modDetailScreen">
			...
			<div class="w3-bar">
				...
				<a class="w3-bar-item w3-button w3-grey mc-tabBtn" id="modTabBtn5" onclick="switchTabPage('mod',5)"><i class="fas fa-file-image fa-fw"></i></a>
			</div>
			<div id="modTabPage1">
				<table class="w3-table">
					<tr>
			  ...
			  <tr>
			  	...
			  	<td><span id="mod<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[12][0] %>">
			  		<% if(patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[12][0]).getFormValue().length()>0){ %>
			  		<img src="/EMS/<%= patGrpBean.getField(EmsDB.EM_PER_PERSONAL_PARTICULAR[12][0]).getFormValue() %>" width="200">
			  		<% } %></span>
			...
			<div id="modTabPage2" style="display:none">
			  ...
			  <%	LivBean modLivBean = new LivBean(); 
			  	...
			  	<tr>
			  		<td><span class="w3-text-red mc-italic">床號: <%=modLivBean.getField(EmsDB.EM_LIV_RECORD[2][0]).getMsg() %></span></td>
			  		<td>
			  			<select name="<%= EmsDB.EM_LIV_RECORD[2][0] %>">
			  				<option value=""></option>
						<% 
			  	...
			  	<tr>
			  		<td><span class="w3-text-red mc-italic">入住日期: <%=modLivBean.getField(EmsDB.EM_LIV_RECORD[5][0]).getMsg() %></span></td>
			  		<td>
			  			<div class="flatpickr date-container">
				  	...
				  	<td><span class="w3-text-red mc-italic"><%= EmsDB.EM_LIV_RECORD[7][1] %>: <%=modLivBean.getField(EmsDB.EM_LIV_RECORD[7][0]).getMsg() %></span></td>
			  	...
			  	<tr>
				  	<td><span class="w3-text-red mc-italic"><%= EmsDB.EM_LIV_RECORD[8][1] %>: <%=modLivBean.getField(EmsDB.EM_LIV_RECORD[8][0]).getMsg() %></span></td>
			...
			<div id="modTabPage4" style="display:none">
				...
				<table class="w3-table-all" id="modDepositScreenTable">
				  	...
				  	<tr>
				  		<td>追加金額: <span class="w3-text-red mc-italic"><%=transBean.getField(EmsDB.EM_TRA_TRANSACTIONS[9][0]).getMsg() %></span></td>
				  		<td><input type="text" name="<%= EmsDB.EM_TRA_TRANSACTIONS[9][0] %>" value="<%=transBean.getField(EmsDB.EM_TRA_TRANSACTIONS[9][0]).getFormValue() %>" maxlength="<%= EmsDB.EM_TRA_TRANSACTIONS[9][3] %>" class="mc-fw" style="max-width:10em">(港幣)</td>
				  		<td>付款方法: <span class="w3-text-red mc-italic"><%=transBean.getField(EmsDB.EM_TRA_TRANSACTIONS[10][0]).getMsg() %></span></td>
				  		<td>
					  		<select name="<%= EmsDB.EM_TRA_TRANSACTIONS[10][0] %>" style="width:150px;">
			...
			<div id="modTabPage5" style="display:none">
				<table class="w3-table">
					<tr>
						<th class="color1 w3-xlarge">新入住 - 上存檔案<span class="w3-text-red mc-italic"><%=patGrpBean.getMsg() %></span></th>
					</tr>
		...
		<div style="display:none" id="delPatientScreen">
			<table class="w3-table-all" id="delPatientScreenTable">
				...
				<tr>
			  		<td>狀態:</td>
			  		<td><span id="cancelLIV_STATUS">-</span></td> 
			  		<td>入住編號:</td>
			  		<td><span id="cancelLIV_ID">-</span></td>  
			  	</tr>
			  	<tr>
			  		<td>中文姓名:</td>
			  	...
			  	<tr><td colspan="4" class="w3-xlarge mc-bold mc-underline">退院資料</td></tr>
			  	<tr>
			  		<td>戶口結餘:</td>
			  		<td><span id="cancelLIV_BALANCE">-</span></td>
			  	</tr>			  	
			  	<tr>
			  		<td>退院原因: <span class="w3-text-red mc-italic"><%=livBean.getField(EmsDB.EM_LIV_RECORD[10][0]).getMsg() %></span></td>
			  		<td>
				  		<select name="<%= EmsDB.EM_LIV_RECORD[10][0] %>" style="width:150px;">
							<% if(livBean.getField(EmsDB.EM_LIV_RECORD[10][0]).getFormValue().equals("")){ %>
								<option value="" selected></option>
							<% }else{ %>
								<option value=""></option>
							<% } %>
							<% if(livBean.getField(EmsDB.EM_LIV_RECORD[10][0]).getFormValue().equals("1")){ %>
								<option value="1" selected>身故</option>
							<% }else{ %>
								<option value="1">身故</option>
							<% } %>
							<% if(livBean.getField(EmsDB.EM_LIV_RECORD[10][0]).getFormValue().equals("2")){ %>
								<option value="2" selected>轉院</option>
							<% }else{ %>
								<option value="2">轉院</option>
							<% } %>
							<% if(livBean.getField(EmsDB.EM_LIV_RECORD[10][0]).getFormValue().equals("3")){ %>
								<option value="3" selected>自行退院</option>
							<% }else{ %>
								<option value="3">自行退院</option>
							<% } %>
					</select>
			  		</td>  
			  	</tr>
			  	<tr>
					<td>退院日期: <span class="w3-text-red mc-italic"><%=livBean.getField(EmsDB.EM_LIV_RECORD[6][0]).getMsg() %></span></td>
			  		<td>
			  			<div class="flatpickr date-container">
		   					<input type="text" name="<%= EmsDB.EM_LIV_RECORD[6][0] %>" value="<%=livBean.getField(EmsDB.EM_LIV_RECORD[6][0]).getFormValue() %>" maxlength="<%=EmsDB.EM_LIV_RECORD[6][3] %>" size="12" data-input>
		   					<a class="mc-button-2 icon" data-clear>&times;</a>
						</div>
			  		</td> 
				  	<td>退院備註: <span class="w3-text-red mc-italic"><%=livBean.getField(EmsDB.EM_LIV_RECORD[11][0]).getMsg() %></span></td>
				  	<td><input type="text" name="<%= EmsDB.EM_LIV_RECORD[11][0] %>" value="<%=livBean.getField(EmsDB.EM_LIV_RECORD[11][0]).getFormValue() %>" maxlength="<%= EmsDB.EM_LIV_RECORD[11][3] %>" class="mc-fw" style="max-width:20em"></td>
				</tr>
			</table>
			<a class="mc-button color4 w3-large" onclick="showConfirmMessage('是否確定退院?','w');"><i class="fas fa-check"></i>確定</a>
			<a class="mc-button color4 w3-large" onclick="showScreen('enqDetailScreen');"><i class="fas fa-times"></i>返回</a>
		</div>
	  	<input type="hidden" name="<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[27][0] %>" value="Y">
	  	<input type="hidden" name="<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[11][0] %>" value="P">
		...
		<div style="display:none" id="enqDetailScreen">
			...
			<div id="enqTabPage2" style="display:none">
				...
				<table class="w3-table-all" id="enqDetailTabPage2ScreenTable">
					...
					<col width="40%">
				  	...
				  	<tr>
				  		<td>銀行(自動轉賬用)</td>
				  		<td><span id="detailBAN_ID"></span></td> 
					  	<td>戶口號碼(自動轉賬用)</td>
					  	<td><span id="detailPER_BANK_ACC_NO">-</span></td>
				  	</tr>
				  	<tr><td colspan="4" class="w3-xlarge mc-bold mc-underline">評核資料</td></tr>
				  	<tr>
				  		<td>LDS編號</td>
			...
			<div id="enqTabPage5" style="display:none">
				<table class="w3-table"><tr><th class="color1 w3-xlarge">訂位 - 列印按金單<a class="mc-button color4 w3-right" onclick="printSlip()"><i class="fas fa-print"></i>列印</a></th></tr></table>
				<div style="overflow-x:auto;">
			  		<iframe id="slipFrame" src="/EMS/jsp/INC/slip-confirm.jsp?dummy=<%=Math.random() %>" style="width:210mm; height:500px; border: 0;"></iframe>
			  	</div>				
			</div>
  			<a class="mc-button color4 w3-large" onclick="showScreen('enqScreen');"><i class="fas fa-times"></i>返回</a>
		</div>
...
var bankBeanList = [ 
	<% for(int i=0;i<bankBeanList.size();i++){ %>
	{
		<% for(int j=0;j<bankBeanList.get(i).getFields().size();j++){
			String fieldName = bankBeanList.get(i).getFields().get(j).getName();%>
			<%=fieldName%>:"<%=bankBeanList.get(i).getFields().get(j).getFormValue()%>"
			<% if(j<bankBeanList.get(i).getFields().size()-1){ %>
				,		
			<% } %>
		<%}%>
	}
	<% if(i!=bankBeanList.size()-1){ %>,<% } %>
	<% } %>];
...
function copyQuoteRecord(recordNum){
    ...
    document.forms[2].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[24][0] %>.value = quoteBeanList[recordNum].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[24][0] %>;
    document.forms[2].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[25][0] %>.value = quoteBeanList[recordNum].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[25][0] %>;
    document.forms[2].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[26][0] %>.value = quoteBeanList[recordNum].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[26][0] %>;
...
function copyReserveRecord(recordNum){
    ...
    document.forms[2].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[24][0] %>.value = reserveBeanList[recordNum].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[24][0] %>;
    document.forms[2].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[25][0] %>.value = reserveBeanList[recordNum].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[25][0] %>;
    document.forms[2].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[26][0] %>.value = reserveBeanList[recordNum].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[26][0] %>;
...
function getAjaxData(enqPatId){
	...
	xhr.onload = function() {
	    if (xhr.status === 200) {
	        ...
	        if(obj.perImageLink != ""){
		        document.getElementById("briefPER_IMAGE_LINK").innerHTML = "<img src='/EMS/"+obj.perImageLink+"' width='200'>";
		        document.getElementById("detailPER_IMAGE_LINK").innerHTML = "<img src='/EMS/"+obj.perImageLink+"' width='200'>";
		        document.getElementById("mod<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[12][0] %>").innerHTML = "<img src='/EMS/"+obj.perImageLink+"' width='200'>";
	        ...
	        document.getElementById("cancelLIV_ID").innerHTML = obj.livId;
        	document.forms[3].<%= EmsDB.EM_LIV_RECORD[0][0] %>.value = obj.livId;
        	document.forms[4].<%= EmsDB.EM_LIV_RECORD[0][0] %>.value = obj.livId;
	        ...
	        document.getElementById("cancelPER_CHI_NAME").innerHTML = obj.perChiName;
	        ...
	        document.getElementById("cancelPER_ENG_NAME").innerHTML = obj.perEngName;
	        ...
	        document.forms[4].<%= EmsDB.EM_PAT_PATIENT_INFO[0][0] %>.value = obj.patId;
	        ...
	        if(obj.livStatus == 'Y'){
	        	...
	        	document.getElementById("cancelLIV_STATUS").className = 'w3-text-green mc-bold';
	        	document.getElementById("cancelLIV_STATUS").innerHTML = "入住中";
	        }else{
	        	...
	        	document.getElementById("cancelLIV_STATUS").className = 'w3-text-red mc-bold';
	        	document.getElementById("cancelLIV_STATUS").innerHTML = "已退院";
	        }
	        
	        if(obj.perGender == 'M'){
		        document.getElementById("briefPER_GENDER").innerHTML = '男';
		        document.getElementById("detailPER_GENDER").innerHTML = '男';
		    	document.getElementById("cancelPER_GENDER").innerHTML = '男';
	        	...
	        	document.getElementById("cancelPER_GENDER").innerHTML = '女';
	        }else{
	        	document.getElementById("briefPER_GENDER").innerHTML = '';
	        	document.getElementById("detailPER_GENDER").innerHTML = '';
	        	document.getElementById("cancelPER_GENDER").innerHTML = '';
	        }
	        ...
	        document.getElementById("cancelPER_HKID").innerHTML = obj.perHKID;
	    	...
	    	document.getElementById("cancelPER_NBIRTH").innerHTML = obj.perNBirth +" ("+getAge(obj.perNBirth) +"歲)";
        	...
        	document.getElementById("cancelPER_LBIRTH").innerHTML = obj.perLBirth;
			...
			for(i=0;i<bankBeanList.length;i++){
				if(bankBeanList[i].<%= EmsDB.EM_BAN_BANK_CODE[0][0] %> == obj.perBankId)
	        		document.getElementById("detailBAN_ID").innerHTML = bankBeanList[i].<%= EmsDB.EM_BAN_BANK_CODE[1][0] %>;
			}
	        document.forms[3].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[24][0] %>.value = obj.perBankId;
	        document.getElementById("detailPER_BANK_ACC_NO").innerHTML = obj.perBankAccNo;
	        document.forms[3].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[25][0] %>.value = obj.perBankAccNo;
	        document.forms[3].<%= EmsDB.EM_PER_PERSONAL_PARTICULAR[26][0] %>.value = obj.perBankAccName;
        	...
        	document.getElementById("transListTable").innerHTML = "<tr><td>入賬日期</td><td>項目</td><td>繳費方法</td><td>交易編號</td><td>金額 (港幣)</td><td>狀態</td><td>備註</td><td>按金單</td></tr>";
        	
        	if(obj.traId!=null && obj.traId.length>0){
	        	for(i=0;i<obj.traId.length;i++){
	        		if(obj.traMethod[i]=="1")
	        			methodStr = "現金";
	        		else if(obj.traMethod[i]=="2")
	        			methodStr = "支票";
	        		else if(obj.traMethod[i]=="3")
	        			methodStr = "自動轉賬";
	        		else
	        			methodStr = "";
	
	        		if(obj.traStatus[i]=="Y")
	        			statusStr ="未結算";
	        		else if(obj.traStatus[i]=="N")
	        			statusStr ="已取消";
	        		else if(obj.traStatus[i]=="C")
	        			statusStr ="已結算";
	        		else
	        			statusStr ="";
	        				
	        		document.getElementById("transListTable").innerHTML = document.getElementById("transListTable").innerHTML + "<tr><td>"+obj.traDate[i]+"</td><td>"+obj.traName[i]+"</td><td>"+methodStr+"</td><td>"+obj.traId[i]+"</td><td>"+obj.traAmount[i]+"</td><td>"+statusStr+"</td><td>"+obj.traNote[i]+"</td><td><a class='mc-button color4' onclick='printPdf(\"/EMS/file/org1001/per100005/pdf/a.pdf\");'><i class='fas fa-edit'></i>列印</a></td></tr>";
	        	}
        	}
    		document.getElementById("detailLIV_BALANCE").innerHTML = obj.accDeposit;
    		document.getElementById("cancelLIV_BALANCE").innerHTML = obj.accDeposit;
	        ...
	        //slip detail
	    	ifWin.document.getElementById("detailPER_CHI_NAME").innerHTML = document.getElementById("detailPER_CHI_NAME").innerHTML;
	    	ifWin.document.getElementById("detailLIV_ID").innerHTML = document.getElementById("detailLIV_ID").innerHTML;
	    	ifWin.document.getElementById("detailLIV_START_DATE").innerHTML = document.getElementById("detailLIV_START_DATE").innerHTML;
	    	ifWin.document.getElementById("detailSLP_DATE").innerHTML = new Date().toJSON().slice(0,10).replace(/-/g,'-');
	    	...
	    	if(obj.traId!=null && obj.traId.length>0){
	        	for(i=0;i<obj.traId.length;i++){
	        		if(obj.traMethod[i]=="1")
	        			...
	        			methodStr = "";
	        		sum = sum + parseFloat(obj.traAmount[i]);
	        		ifWin.document.getElementById("transListTable").innerHTML = ifWin.document.getElementById("transListTable").innerHTML + "<tr><td>"+obj.traId[i]+"</td><td>"+obj.traDate[i]+"</td><td>"+obj.traName[i]+"</td><td>" + methodStr + "</td><td style='text-align:right'>"+ obj.traAmount[i]+"</td></tr>" ;
	        	}
	    	}
	    	ifWin.document.getElementById("detailRES_BALANCE").innerHTML = sum.toFixed(2);
	    	ifWin.document.getElementById("detailRES_BALANCE2").innerHTML = sum.toFixed(2);
	    	ifWin.document.getElementById("detailRES_BALANCE3").innerHTML = sum.toFixed(2);
	    	
	    	//slip trailer
	    	ifWin.document.getElementById("detailPER_CHI_NAME2").innerHTML = document.getElementById("detailPER_CHI_NAME").innerHTML;
	    	ifWin.document.getElementById("detailRES_ID2").innerHTML = document.getElementById("detailLIV_ID").innerHTML;
	    	ifWin.genEmptyLine(obj.traId.length);
	    }
	    else {
...
if(document.forms[2].<%= EmsDB.EM_TRA_TRANSACTIONS[8][0] %>.value == ""){
	document.forms[2].<%= EmsDB.EM_TRA_TRANSACTIONS[8][0] %>.value = today;
}
if(document.forms[3].<%= EmsDB.EM_TRA_TRANSACTIONS[8][0] %>.value == ""){
	document.forms[3].<%= EmsDB.EM_TRA_TRANSACTIONS[8][0] %>.value = today; 
}
if(document.forms[4].<%= EmsDB.EM_LIV_RECORD[6][0] %>.value == ""){
	document.forms[4].<%= EmsDB.EM_LIV_RECORD[6][0] %>.value = today; 
}
...
function printPdf (url){
	...
	if (isIE){
		if (pFrame == null){
			pFrame = document.createElement('EMBED');
			pFrame.id = "printFrame";
			pFrame.width=0;
			pFrame.height=0;
			pFrame.type="application/pdf";
			document.body.appendChild(pFrame);
			pFrame.src = url;
			setTimeout(function(){ pFrame.print(); }, 1000);
		} else {
			pFrame.src = url;
			setTimeout(function(){ pFrame.print(); }, 1000);
		}
		...
		if (pFrame == null){
			...
			document.body.appendChild(pFrame);
			pFrame.src = url;
			setTimeout(function(){ pFrame.focus();pFrame.contentWindow.print(); }, 1000);
		} else {
			pFrame.src = url;
			setTimeout(function(){ pFrame.focus();pFrame.contentWindow.print(); }, 1000);
		}
...
printPdf11 = function (url) {
	...
	if (!this._printIframe) {
		...
		if ( window.addEventListener ) { 
			alert(1);
			iframe.addEventListener( "load", myLoad, false );
		  }
		  else if ( window.attachEvent ) {
			  alert(2);
			  iframe.attachEvent( "onload", myLoad );
		  }
		  else {
			  alert(3);
			  iframe.onload = myLoad;
		  }
